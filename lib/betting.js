...
/**
 * P(X >= need) for X ~ Binomial(trials, p), computed with a stable iterative
 * term so we never build large factorials. Used to score how likely a bid is
 * true given the unknown dice on the table.
 */
export function probabilityAtLeast(need, trials, p) {
  if (need <= 0) return 1;
  if (need > trials) return 0;

  let cumulativeBelow = 0; // P(X <= need - 1)
  let term = (1 - p) ** trials; // P(X = 0)
  for (let k = 0; k < need; k++) {
    cumulativeBelow += term;
    term *= ((trials - k) / (k + 1)) * (p / (1 - p)); // P(X=k) -> P(X=k+1)
  }
  return 1 - cumulativeBelow;
}

/**
 * Probability that a bet is true given our own dice: we know our matches for
 * certain and model the remaining unknown dice as a binomial draw.
 */
export function probabilityBetTrue(dice, bet, totalDice) {
  const known = countMatching(dice, bet.value);
  const unknown = totalDice - dice.length;
  return probabilityAtLeast(bet.count - known, unknown, matchProbability(bet.value));
}
