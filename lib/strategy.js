...
export class SimpleDudoStrategy extends Strategy {
  ...
  decide(state) {
    const { myDice, totalDice, currentBet } = state;
    const claim = this.#strongestSafeClaim(myDice, totalDice, currentBet);
  ...
  /**
   * Highest-ranked legal raise whose probability of being true is at least
   * `threshold`, or null if none qualifies. A claim can't exceed the dice on
   * the table, so counts are capped at `totalDice`.
   */
  #strongestSafeClaim(dice, totalDice, currentBet) {
    let best = null;
    let bestRank = -1;
    ...
    for (let value = MIN_FACE; value <= MAX_FACE; value++) {
      for (let count = 1; count <= totalDice; count++) {
        ...
        if (probabilityBetTrue(dice, claim, totalDice) < this.threshold) continue;
