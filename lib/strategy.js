 ...
 * *when to challenge* by reading the profile of whoever bet immediately before
 * us (the standing bidder) rather than probability alone.
 *
 * The intuition from our game logs: some teams bluff almost every time they're
 * challenged (e.g. Bot-A ~0.78, MyTeam ~0.74) while others almost never do
 * (e.g. Fable Wins ~0.34). Against a habitual bluffer we should pull the trigger
 * on thinner evidence; against an honest bidder we should give them more rope.
 *
 * How the standing bidder's profile is used (see {@link #callBar}):
 *   - Confidence-weighted bluff estimate: the observed `bluffRate` is shrunk
 *     toward a neutral 0.5 prior by how many of their rounds we've actually
 *     resolved (`outcomesKnown`). This replaces a hard "trust after N rounds"
 *     cliff with a smooth ramp, so a strong cross-game profile is exploited
 *     early without over-reacting to a two- or three-round sample.
 *   - Timeout awareness: a forced timeout bet was never *chosen* by the bidder,
 *     so their bluff history says nothing about it — we ignore the shading and
 *     fall back to the neutral bar for that bet.
 *
...
export class OpponentAwareDudoStrategy extends Strategy {
  constructor({
    ...
    // Pseudo-count for the shrinkage prior on a bidder's bluff rate: with
    // `priorStrength` resolved rounds we weight their observed rate 50/50 with
    // the neutral prior, trusting it more as evidence accumulates. Higher =
    // more skeptical of small samples.
   ...
   * call sooner), an honest bidder nudges it down.
   *
   * Rather than a hard "trust after N rounds" cutoff, we shrink the bidder's
   * observed `bluffRate` toward a neutral 0.5 prior in proportion to how many of
   * their rounds we've resolved, so the profile ramps in smoothly and a strong
   * cross-game history is used from the first turn. A forced timeout bet carries
   * no bluff signal, so we skip the shading for it.
   */
  #callBar(state) {
    const stats = this.#lastBidderStats(state);
    if (!stats || stats.lastBetWasTimeout) {
      return this.callThreshold;
    }

    const n = Math.max(0, stats.outcomesKnown ?? 0);
    const confidence = n / (n + this.priorStrength);
    const effectiveBluff =
      confidence * stats.bluffRate + (1 - confidence) * 0.5;

    const bar =
      this.callThreshold + this.bluffWeight * (effectiveBluff - 0.5);
    return Math.min(0.95, Math.max(0.05, bar));
  }

  /**
   * Profile of whoever made the bet we're now responding to (the player who bet
   * immediately before us), or null. We tag on whether that standing bet was a
   * forced timeout, since a timeout bet isn't a chosen bid and so shouldn't be
   * read through the bidder's bluff history.
   */
  #lastBidderStats(state) {
    ...
    if (!last) return null;
    const stats = this.opponents.statsFor(last.teamName ?? last.playerId);
    if (!stats) return null;
    return { ...stats, lastBetWasTimeout: !!last.isTimeout };
  }
}
