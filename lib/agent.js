...
export class RollerAgent {
  ...
  constructor({
    ...
    this.#log = logger;
    this.#betStore = betStore ?? null;
    // Let the store exclude our own bets once identity is known. `state` is
    // updated in place, so keep a live view rather than a snapshot.
    if (this.#betStore) this.#betStore.self = this.#state;
  }
