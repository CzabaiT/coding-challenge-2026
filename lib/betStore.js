...
/**
 * Persists other players' bets to disk in a shape an AI agent can consume:
 * a raw event {@link BetStore#recordBet log} plus per-opponent
 ...
 *     agent.
 *
 * Our own bets are excluded: identity is read from {@link BetStore#self}.
 */
export class BetStore {
  ...
  /** True when the given identity is us (and should be skipped). */
  ...
  /**
   * Record a single opponent bet. Skips our own bets. Appends to the event log
   * with a per-round sequence number and updates the opponent's running
   * totals, then schedules a debounced flush.
   */
  recordBet({ gameId, round, teamName, playerId, bet, isTimeout = false } = {}) {
    if (!teamName && !playerId) return;
    if (this.#isSelf(teamName, playerId)) return;
